...
$(document).ready(function() {
	...
	// 	if ((Math.abs(e.clientX - btUpX) < btUpArea && Math.abs(e.clientY - btUpY) < btUpArea) || e.clientX + 50 > btUpX) {
	// 		topBtUp = (e.clientY - 100) < 100 ? 110 : e.clientY - 50;
	// 		topBtUp = (topBtUp + 200) > $(window).height() ? $(window).height() - 155 : topBtUp;
	// 		leftBtUp = (e.clientX + 60) > $(window).width() ? $(window).width() - 115 : e.clientX - 50;
	// 		$('#bt-up').stop().animate({top: topBtUp, left: leftBtUp, opacity:1}, 200, 'easeOutQuad');
	// 		isAnimatingBtUp = false;
	// 	}
	...
	$('#header nav a').click(function() {
		...
		if ($(this).data('title') && $(this).data('title') != '') {
			document.title = $(this).data('title') + ' - ' + $('#header h1 a').text();
		} else {
			document.title = $('#header h1 a').text();
		}

		History.pushState({ page: $(this).data('rel') }, document.title, $(this).attr('href'));

		return false;
	});
	
	// Change current page onload
	if ( $('body').data('page') && $('body').data('page') != '' ) {
		var page = $('body').data('page');
		var explode = page.split('/');
		if ( explode.length > 0 ) {
			page = explode[0];

			// var doc = $('#page-documentation dl dt[content="' + explode[1] + '"]');
			// if ( doc.length > 0 ) {
			// 	$('body,html').animate({ top: doc.offset().top }, 200);
			// };
		}
		$('#header nav a[data-name="' + page + '"]').trigger('click');
	};
